  ...
  describe('Test Case 15: Place Order with Prior Registration', () => {
    let driver;
    ...
    test('Place order after registration', async () => {
  await driver.get('https://automationexercise.com/');
  await driver.findElement(By.xpath('//a[@href="/products"]')).click();
  await driver.findElement(By.xpath('//a[@href="/product_details/1"]')).click();
  const button = await driver.findElement(By.xpath('//button[@class="btn btn-default cart"]'));
  ...
  await driver.wait(until.elementIsEnabled(continueButton), 5000);

  // Attempt to click the button, fallback to JavaScript if needed
  try {
  ...
  await driver.wait(
    until.elementLocated(By.xpath('//p[text()="Congratulations! Your order has been confirmed!"]')),
    5000
  );
  const orderSuccess = await driver.findElement(
